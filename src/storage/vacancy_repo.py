...
# runs/day; Arbeitnow and DOU hourly, 24/day; Djinni every 4h, 6/day;
# justjoin.it daily, 1/day). Instead each threshold targets roughly the
# same real-world window regardless of source: ~1 day missing -> "stale"
# (shown, with a badge), ~3 days missing -> "removed" (hidden from
# recommendations). justjoin.it and Djinni are both more conservative
# than a literal 1x/3x day count for the same underlying reason, even
# though it shows up differently for each: justjoin.it only runs once a
# day, so a single missed run there is far more likely to be page-
# ordering/pagination noise than the same miss would be for a source
# re-checking every 30-60 minutes; Djinni has no bulk endpoint and caps
# how many postings a single run can even see (libs/djinni-scraper's
# max_pages - confirmed live today's real PHP total fits with room to
# spare, but there's no guarantee that holds forever), so a single run
# not seeing a given posting is weaker evidence than for a source that
# reads its whole category in one request. DOU was originally left out
# of this dict entirely - a real gap, caught after building Djinni's
# thresholds and going back to check: without an entry here every DOU
# posting fell back to the generic default (1/3), meaning "stale" after
# 1 missed hourly run and "removed" after 3 (3 hours) rather than the
# ~1/~3 *day* window every other source actually gets.
STALE_AFTER_MISSED_RUNS = {"adzuna": 48, "arbeitnow": 24, "dou": 24, "justjoinit": 2, "djinni": 12}
REMOVED_AFTER_MISSED_RUNS = {"adzuna": 144, "arbeitnow": 72, "dou": 72, "justjoinit": 5, "djinni": 30}
